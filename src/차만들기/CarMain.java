...
import static 차만들기.Common.DISTANCE;

public class CarMain {
    public static void main(String[] args) {
        Car car = null;
        Scanner sc = new Scanner(System.in);
        int location = 0, carType = 0, weather = 0, option = 0;
        while (true) {
            System.out.println("이동 지역 [1]부산 [2]대전 [3]강릉 [4]광주 : ");
            location =sc.nextInt();
            if(location > 0 && location < 5) break;
            System.out.println("이동할 지역 선택이 잘 못되었습니다.");
        }
        System.out.println("이동할 승객 수 : ");
        ...
        while (true) {
            ...
            System.out.println("차량 선택이 잘 못되었습니다.");
        }
        while (true) {
            System.out.print("부가 기능 [1]ON [2]OFF : ");
            option = sc.nextInt();
            if(option == 1 || option == 2) break;
            System.out.println("모드 선택이 잘 못되었습니다.");
        }
        boolean isMode = (option == 1) ? true : false;
        switch (carType) {
            case 1 : car = new SportsCar("포르쉐 911"); break;
            case 2 : car = new Sedan("제네시스 GV80"); break;
            case 3 : car = new Bus("고속버스"); break;
            default:System.out.println("차량 선택이 잘 못되었습니다.");
        }
        while (true) {
            System.out.print("날씨 선택 [1]맑음 [2]비 [3]눈 : ");
            weather = sc.nextInt();
            if(weather > 0 && weather < 4) break;
            System.out.println("날씨 선택이 잘 못되었습니다.");
        }
        if(car != null){
            car.setMode(isMode);
